...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const { min, max, fixed, useDarList } = await getDarResult(interaction.user.id, "bidar");
      ...
      if (!useDarList) {
        ...
        if (darList) {
          ...
          if (bidarEntry) {
            ...
            darList.bidar.push({ userid, meter });
            await darList.save();
          }
        } else {
          meter = applyDarRange(min, max);
          ...
          darList = new DarList({ bidar: [{ userid, meter }] });
          await darList.save();
        }
      }
    } catch (err) {
    ...
    await addDarHistory(interaction.user.id, "bidar", meter);
