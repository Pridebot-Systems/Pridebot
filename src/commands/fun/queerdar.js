...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const { min, max, fixed, useDarList } = await getDarResult(interaction.user.id, "queerdar");
      ...
      if (!useDarList) {
        ...
        if (darList) {
          ...
          if (queerdarEntry) {
            ...
            darList.queerdar.push({ userid, meter });
            await darList.save();
          }
        } else {
          meter = applyDarRange(min, max);
          ...
          darList = new DarList({ queerdar: [{ userid, meter }] });
          await darList.save();
        }
      }
    } catch (err) {
    ...
    await addDarHistory(interaction.user.id, "queerdar", meter);
