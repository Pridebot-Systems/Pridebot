...
try {
  ...
  console.warn('Redis not available for queue, background processing disabled:', error.message);
  avatarQueue = null;
}
...
const validFlags = [
  ...
  "butch", "butchlesbian", "butchlesbian2", "butchlesbian3", "catgender",
  "cupioromantic", "demibisexual", "demiboy", "demigirl", "deminonbinary",
  "demiromantic", "demisexual", "gay", "genderfae", "genderfaun", "genderfluid",
  "genderflux", "genderqueer", "girlflux", "graygender", "grayromantic",
  "graysexual", "lesbian", "lesboy", "lgbt", "lunarian", "neptunic", "nonbinary",
  "omnisexual", "pangender", "pansexual", "polyamorous", "polysexual", "queer",
  "queerplatonic", "queerplatonic2", "sapphic", "selenosexual", "singularian",
  "solarian", "spacialian", "stellarian", "transfeminine", "transgender", "transmasculine"
];

class AvatarProcessor {
  constructor() {
    this.initialized = false;
    this.processing = new Map(); // Track ongoing processes to prevent duplicates
  }
  ...
  async initialize() {
    ...
    try {
      ...
      console.log(`Avatar Processor initialized in ${Date.now() - startTime}ms - ${successCount}/${validFlags.length} flags loaded`);
  ...
  // Get cached flag buffer
  getFlagBuffer(flagName) {
  ...
  async generateAvatar(avatarURL, flagName, flagName2, userID, username = null) {
    ...
    const processingPromise = this._processAvatar(avatarURL, flagName, flagName2, userID, cacheKey);
  ...
  async _processAvatar(avatarURL, flagName, flagName2, userID, cacheKey) {
    ...
    try {
      ...
      if (!flagBuffer) {
        throw new Error(`Flag buffer not found: ${flagName}`);
      }
      if (flagName2 && !flagBuffer2) {
        throw new Error(`Flag buffer not found: ${flagName2}`);
      }
      ...
      if (processingTime > 1000) {
        console.log(`Slow avatar generation: ${processingTime}ms for ${userID} (${flagName}${flagName2 ? '+' + flagName2 : ''})`);
      }
  ...
  async saveAvatar(avatarData, userID, fileName, username = null) {
    ...
    try {
      ...
      if (username) {
        const usernameDir = path.join(__dirname, "../../pfps", username.toLowerCase());
        await fs.mkdir(usernameDir, { recursive: true });
        
        const usernameBasePath = path.join(usernameDir, fileName.replace('.png', ''));
...
if (avatarQueue) {
  avatarQueue.process('generate', async (job) => {
    ...
    return await processor.generateAvatar(avatarURL, flagName, flagName2, userID);
  });
}
